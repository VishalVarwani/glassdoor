...
async function scrapePage(pageNumber, jobTitle, location) {
    ...
    const pageUrl = `https://www.glassdoor.de/Job/${location.trim().replace(/\s+/g, '-').toLowerCase()}-${jobTitle.trim().replace(/\s+/g, '-').toLowerCase()}-jobs-SRCH_IL.0,29_KO30,47.htm?p=${pageNumber}`;
